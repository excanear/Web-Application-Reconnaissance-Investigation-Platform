...
- The `table` format's rendered output must stay behaviorally identical to today's — verified by the existing `report`/`table`-format tests in `test_cli.py` passing unmodified after the `report_data.py` extraction. Exception, reconciled with the EPSS-tie-break constraint below: CVE *row order* may differ from pre-Fase-F output when two CVEs share the same CVSS score but different EPSS scores (today's CVSS-only sort would keep them in query order; the new CVSS-then-EPSS sort tie-breaks them by EPSS instead). No pre-existing test triggers this — confirmed by two independent reviews — and cell content/formatting for every row stays identical either way. This is the one deliberate, documented exception to "identical."
